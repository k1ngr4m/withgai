...
#### A. First Playable 必做
- 后端职业可玩：`后端`
- 其他职业先锁定为未解锁状态：`前端 / 测试 / 算法 / 产品经理 / HR`
- 完整章节树图与楼层路径选择
- 基础战斗闭环
- 奖励、商店、休息处、事件、Boss
- 局外成长：工位升级 + 职业解锁树
...
#### B. 扩展预留
- `前端 / 测试 / 算法 / 产品经理 / HR` 完整系统占位
- 当前阶段不继续开发锁定职业的战斗接入、奖励池、商店池与平衡内容
- 更多敌人、事件、遗物与职业特化效果
- 更复杂的效果 DSL
...
### 1.4 对 Brief 的工程覆盖
`01_GAME_DESIGN_BRIEF.md` 定义六职业长期设计方向。为避免实现范围歧义，本文件在工程层面明确当前阶段为：

- `First Playable` 以后端可玩为准
- `前端 / 测试 / 算法 / 产品经理 / HR` 只展示为锁定未解锁职业
- 锁定职业暂不进入实际战斗、奖励、商店与平衡开发
- 不重写 `01` 的玩法真源地位
...
当前 First Playable 只接通后端资源状态。其他职业资源状态仅作为锁定未解锁职业的后续预留，不进入当前阶段开发。
...
#### 职业资源面板差异
- 后端为当前 First Playable 唯一实际显示资源面板
- 后端：显示 `服务 / 缓存`
- 前端 / 测试 / 算法 / 产品经理 / HR：仅作为锁定职业预留展示，不进入当前战斗 UI
...
### 14.1 必做
- 后端职业可玩
- 前端 / 测试 / 算法 / 产品经理 / HR 在职业选择与职业树中显示为锁定未解锁
- 完整树图
- 战斗闭环
- 奖励 / 商店 / 事件 / 休息处 / Boss
- 职业解锁树展示
...
### 15.1 新开一局
从主菜单进入职业选择，选中后端，成功生成章节树图并进入首战。

### 15.2 锁定职业展示
前端、测试、算法、产品经理和 HR 在职业选择与职业树中可见，但处于锁定未解锁状态，不能开始单局，且不进入奖励、商店、战斗可选池。
...
- 首个原型以 `后端` 可玩为准
- `前端 / 测试 / 算法 / 产品经理 / HR` 只保留锁定未解锁展示位，不接入首个原型战斗流程
- 当前阶段不继续开发锁定职业；若后续要纳入 First Playable，应作为本文件下一版扩展
